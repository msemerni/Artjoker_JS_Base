...
function checkIsValidPhone(phone: string): boolean {
  return /^\+\d{2}\(\d{3}\)\d{3}(-\d{2}){2}$/g.test(phone);
}

// формат кода страны +с, +сc, +сcc, +с-ccc
function checkIsValidPhoneNumber(phone: string): boolean {
...
function checkIsSite(url: string): boolean {
...
function checkIsValidIpv4(ipv4: string): boolean {
